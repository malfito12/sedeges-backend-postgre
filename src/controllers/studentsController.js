...
controllers.getStudent = async (req, res) => {
    ...
    try {
        const result = await conn.query(
            'SELECT student_first_name,student_last_name,student_birth_date,student_age,student_sex,student_ocupation,student_ci,student_id FROM students WHERE student_ci=$1',
            [req.params.id]
        )
        res.status(200).json(result.rows)
    } catch (error) {
